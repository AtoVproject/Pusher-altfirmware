...
// Keep track of which FSR is pressed
int FSRpressed[] = {
  0, 0, 0, 0
};

// Number of FSR pressed
int FSRn = 0;
...
void loop () {
  ...
  // Build array of FSR pressed + digital array Array for FSR pressed => if Digital Arraw = 1 1 1 1 => Param Else => for statement run each output individually
...
// --------------------------------------------------FSR 1-4--------------------------------------------------------------
  ...
  // Add the value of the activated knobs together
  output1 = Value[0] * FSRpressed[0] + Value[1] * FSRpressed[1] + Value[2] * FSRpressed[2] + Value[3] * FSRpressed[3];
  output2 = Value[4] * FSRpressed[0] + Value[5] * FSRpressed[1] + Value[6] * FSRpressed[2] + Value[7] * FSRpressed[3];
  FSRn = FSRpressed[0] + FSRpressed[1] + FSRpressed[2] + FSRpressed[3];
  
  if (FSRn == 0){
  setOutput(2, 1, GAIN_1, 1, 0);}
  else  {
  setOutput(3, 0, GAIN_1, 1, output1);
  setOutput(3, 1, GAIN_1, 1, output2);
  }
}
